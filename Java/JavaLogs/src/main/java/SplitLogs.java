...
public class SplitLogs {
    public static void main(String[] args) throws IOException {
        ...
        String filesLogPath = args[1]; // получение аргуемента путь сохранения новых файлов
        ...
        if (fileSize > 0) { // проверка пустой ли файл
            ...
            { // считываем файл с учетом кодировки
                for (int i = 1; i < (count_file + 1); i++) // создаем указанное количество фалов для записи
                {
                    ...
                    { // поток записи строк в файл с учетом кодировки
                        long fileSize_n = file_n.length(); // Проверяем размер файла в, который пишем строки

                        String line;
                        while (fileSize_n < fileSize / (count_file) && (line = reader.readLine()) != null)
                        { //проверяем размер файла в котоый пишем и естли не прочитанные строки в исходном файле
                            writer.write(line + "\n"); // пишем строки в файл
